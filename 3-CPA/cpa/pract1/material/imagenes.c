           ...
           int n, int m)
{
  ...
  for (p = 0; p < pasos; p++) {
	  ...
	  //#pragma omp parallel for private(l,v,k, j, tot)
    for (i = 0; i < n; i++) {
		
		//#pragma omp parallel for private(l,v,k,tot)
      for (j = 0; j < m; j++) {
        int resultado_r = 0;
        int resultado_g = 0;
        int resultado_b = 0;
        ...
        //#pragma omp parallel for private(v, l) reduction(+:tot, resultado_r, resultado_g, resultado_b)
        for (k = max(0, i - radio); k <= min(n - 1, i + radio); k++) {
								
		#pragma omp parallel for private(v) reduction(+:resultado_r) reduction(+:resultado_g) reduction(+:resultado_b) reduction(+:tot)
