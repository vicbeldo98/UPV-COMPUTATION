...
{ int i,j,d,im,dm;// dm es la distancia del código más parecido, im su índice
  ...
  tavg, tmax=0;
  tmin=MAX_LON;
  #pragma omp parallel private(i, dm, im, d) reduction(+:tavg)
  {
	  ...
	  printf("Thread %d of %d: Min/Average/Maximun Length: %d/%.2f/%d \n", omp_get_thread_num(), omp_get_num_threads(), min, (avg*1.00)/count, max);
   }
   printf("TOTAL: Min/Average/Maximun Length: %d/%.2f/%d \n", tmin, (tavg*1.00)/nt2, tmax);
}
