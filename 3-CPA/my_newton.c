                   ...
                   int me, int np)
{
  ...
  if(me == 0) {
    ...
    for ( rows_done = 0 ; rows_done < h ; rows_done++ ) {
     ...
     MPI_Test(&req,&ended,&status);

     while(!ended && next_row < h){
         /*procesa la fila n ́umero siguiente_fila*/
        num_row=next_row;
        z0.b = y1 + iy*num_row;
        for ( j = 0 ; j < w ; j++ ) {
            z0.a = x1 + ix*j;
...
1.Cuando un proceso trabajador le env ́ıa una fila al proceso maestro, 
¿c ́omo le indica de qu ́e fila se trata?-> Por el tag
2.¿Como sabe un proceso trabajador que ya no tiene que procesar m ́as filas?-> Porque recibe
una petición de fila fuera de rango de filas.
