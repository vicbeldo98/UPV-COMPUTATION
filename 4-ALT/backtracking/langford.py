...
def langford_directo(N, allsolutions):
    ...
    def backtracking(num):
        if num<=0:
            ...
            cabe=False
            for i in range(N2):
                if(seq[i]==0 and (i+num+1) < N2 and seq[i+num+1]==0):
                    cabe=True
...
def langford_data_structure(N):
    ...
    Y={}
    #Llenas el diccionario Y con pares clave:valor donde los valores
    #son listas que contienen tripletas
    #Tripleta: ['n1','p2','p4']=[value(n),position(p),position(p+(n+1))]
    for v in range(1,N+1):
        for p in range(0,2*N):
            if(p+(v+1)<2*N):
                Y[value(v)+position(p)]=[value(v),position(p),position(p+(v+1))]
